...
if( !class_exists('JobApplyForm') ){
    ...
    public static function get_apply_form( $inline = false, $custom_title = '', $name = '', $show_title = true, $post_id = '' ){
        ...
        $close_img 			= apply_filters('jp-modal-close-image', '<img src="'.JOBPOSTINGSURL.'images/close.svg">');
            ...
            $out .= '<form id="jobs-modal-form" method="post" enctype="multipart/form-data">';
                ...
                $out .= '<div class="jobs-modal-footer">';
                    ...
                    if( $site_key && $secret_key) {
                        $out .= '<input type="hidden" name="captca_response" value="">';
                    }

                    $out .= '<img class="jobs-sending" src="'.JOBPOSTINGSURL.'/images/loading.svg">';
    ...
    public static function get_modal_input( $options = array() ){
        ...
        $out .= '<div class="jobs-modal-input modal-input-'.$key.' '.$type.'_field '.$class.'" data-control="">';
            switch ($type) {
                ...
                case 'textarea':
                    $out .= '<label class="input-label" for="input-'.$key.'">'.$label.$label_req.'</label>';
                    $out .= '<textarea id="input-'.$key.'" type="text" name="input_'.$key.'" data-jobinput="'.$key.'" class="modal-input-text input-'.$key.' '.$req.'" placeholder="'.$placeholder.'"></textarea>';
                    break;
                ...
                case 'file':
                    ...
                    $remove = '<img class="remove" src="'.JOBPOSTINGSURL.'/images/remove.svg">';
                    $add = '<img class="add" src="'.JOBPOSTINGSURL.'/images/add.svg">';
                    ...
                    $out .= '<div id="'.$key.'" class="modal-input-fileinput '.$class.'" data-files="0">';
                        ...
                        if( $multiple ){
                            ...
                            $out .= '<input id="'.$forid.'" type="file" '.$accept.' name="input_'.$key.'" data-jobinput="'.$key.'" class="inputfile modal-input-file input-'.$key.' '.$req.'" placeholder="'.$placeholder.'">';
